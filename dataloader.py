...
def get_dataloader(data_dir, dataset, batch_size, test_batch_size, scalar_type='Standard'):
    ...
    for category in ['train', 'val', 'test']:
        ...
        print('data[x_{}: {}]'.format(category, data['x_' + category].shape))
        print('data[y_{}: {}]'.format(category, data['y_' + category].shape))
    ...
    dataloader = {}
    dataloader['train'] = STDataloader(data['x_train'],
                                       data['y_train'],
                                       batch_size,
                                       shuffle=True
                                       )
    dataloader['val'] = STDataloader(data['x_val'],
                                     data['y_val'],
                                     test_batch_size,
                                     shuffle=False
    )
    dataloader['test'] = STDataloader(data['x_test'],
                                      data['y_test'],
                                      test_batch_size,
                                      shuffle=False,
                                      drop_last=False
